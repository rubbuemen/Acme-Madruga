...
public interface AdministratorRepository extends JpaRepository<Administrator, Integer> {
	...
	@Query("select p1.title, rm1.status, 1.0*count(rm1)/(select count(rm2) from Procession p2 join p2.requestsMarch rm2 where p2 = p1) from Procession p1 join p1.requestsMarch rm1 group by rm1.status, p1 order by p1")
	Collection<Object[]> dashboardQueryC4();

	@Query("select p from Procession p where datediff(p.momentOrganise, CURRENT_DATE) <= 30")
	Collection<Procession> dashboardQueryC5();
